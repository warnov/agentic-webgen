...
if ($outputs) {
    ...
    Write-Host "  Storage Account ID: $($outputs.storageAccountId.value)" -ForegroundColor White
    Write-Host "  Primary Blob Endpoint: $($outputs.primaryBlobEndpoint.value)" -ForegroundColor White
}

Write-Host "🎉 Infrastructure deployment completed successfully!" -ForegroundColor Green
Write-Host "💡 Next steps:" -ForegroundColor Yellow
Write-Host "  1. Upload HTML templates to the 'templates' container" -ForegroundColor White
Write-Host "  2. Configure Azure Functions to use this storage account" -ForegroundColor White
Write-Host "  3. Set up Azure AI Foundry agents" -ForegroundColor White
